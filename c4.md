...
sequenceDiagram
    ...
    AUTH-->>API: Retorna Access Token e ID Token
...
## 2) Registrar Abastecimento e Calcular KM/L
...
sequenceDiagram
    ...
    API->>DB: findOne(ultimo abastecimento anterior do veiculo)
    DB-->>API: Retorna kmAnterior
    API->>API: Calcula KM Rodado = kmAtual - kmAnterior
    API->>API: Calcula KM/L = KM Rodado / litros
    ...
    par Agregações assíncronas
      ...
      JOB->>DB: Atualiza coleções de métricas e visões
    end
```

---

## 3) Iniciar e Encerrar Viagem (Regras de Negócio)
...
sequenceDiagram
    ...
    alt Regras OK
      ...
      API-->>FE: 409/422 (ex.: CNH vencida ou sobreposição)
    ...
    API->>API: Calcula KM Rodado
    API->>API: Atualiza métricas internas
    API->>DB: updateOne(viagens)
    API->>DB: updateOne(veiculos.set quilometragem_atual)
